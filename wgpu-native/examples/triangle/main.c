...
const char* convert_file_to_path(char * destination, const char *executable_dir, const char* filename) {
  snprintf(destination, 1024, "%s/%s", executable_dir, filename);
  return destination;
}

int main(int argc, char *argv[]) {
  // Make a copy because dirname() may modify the input string
  char *executable_file_path = strdup(argv[0]);
  if (!executable_file_path) {
      perror("strdup");
      return 1;
  }
  char *executable_dir = dirname(executable_file_path);
  convert_file_to_path(shader_filepath, executable_dir, "shader.wgsl");
  free(executable_file_path);
...
#error "Unsupported GLFW native platform"
  ...
  WGPUShaderModule shader_module =
      frmwrk_load_shader_module(demo.device, shader_filepath);
